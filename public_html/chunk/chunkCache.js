        ...
        for(var i = yymin2, y = 0; i < yymax2; i++, y++ ){
           ...
           for(var z = 0; z < 16; z++){
               for(var x = 0; x < 16; x+=2){
                   ...
                   index2 = (i+1)*324 + (z+1)*18 + (x+1);
                   cacheId[index2] = asection.blocks[index];
                   cacheId[index2+1] = asection.blocks[index+1];
                   tmp = asection.data[(index/2)];
                   cacheData[index2] = ((tmp) & 0x0F) & block[asection.blocks[index]].mask;
                   cacheData[index2+1] = ((tmp >> 4) & 0x0F) & block[asection.blocks[index+1]].mask;
                   tmp = asection.skyLight[(index/2)];
                   cacheSlight[index2] = (tmp) & 0x0F;
                   cacheSlight[index2+1] = (tmp >> 4) & 0x0F;
                   tmp = asection.blockLight[(index/2)];
                   cacheBlight[index2] = (tmp) & 0x0F;
                   cacheBlight[index2+1] = (tmp >> 4) & 0x0F;
               }
           }
        }
        //fill corners
